...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        #这段不好说，就是做一个流程，具体怎么处理矩阵，三维矩阵，特征图，这就是处理具体步骤。
        #过来一些数据，然后怎么处理过来的数据，过来的处理过的图像，特征图。
        #也是一种设计。设计这个模块。设计完之后，也随便说了前向传播，就跟耿说的一样，形成类实例，调用实例的指针，之后前向传播，也就是数据处理的流程。
        self.dim = dim
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape # 三维
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  #四维

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # 五维
        else:
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  #三维
        x = self.proj(x)
        x = self.proj_drop(x)
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):  #for i in range() 是用来给i赋值
...
class pvt_tiny(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_tiny, self).__init__(
            ...
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 3, 4, 5],
